...
//Declarando a estrura do nó
typedef struct Node  {
	int valor;
	struct Node* prox;
	struct Node* ant;
}node;//node é um alias(codinome, apelido)
...
//Cria um node e retorna um ponteiro desse novo nó
...
//imprime os nós de trás para frente
void Print_Fim() {
	node* temp = head;
	if(temp == NULL){
